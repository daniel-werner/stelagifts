...
from multilingual.flatpages.middleware import FlatpageFallbackMiddleware
...
class FlatpageFallbackMiddleware(FlatpageFallbackMiddleware):
    def process_response(self, request, response):
        ...
        try:
            ...
            f = get_object_or_404(MultilingualFlatPage, url__exact=request.path_info, sites__id__exact=settings.SITE_ID)
            if f: breadcrumbs_for_flatpages(request,f)
            return super(FlatpageFallbackMiddleware, self).process_response(request, response)
